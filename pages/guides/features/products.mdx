...
```bash
curl -X GET "https://api.upay-sistema.onrender.com/api/products?status=ACTIVE" \
  -H "Authorization: Bearer SUA_API_KEY"
```
...
```bash
curl -X PUT "https://api.upay-sistema.onrender.com/api/products/{id}" \
  -H "Authorization: Bearer SUA_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
...
```javascript
const response = await fetch(`https://api.upay-sistema.onrender.com/api/products/${productId}/stock`, {
  method: 'PATCH',
  headers: {
    'Authorization': `Bearer ${apiKey}`,
    'Content-Type': 'application/json'
